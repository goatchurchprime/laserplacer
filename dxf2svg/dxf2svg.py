...
def outent(e, fwrite, col, layername, th):
    if e.dxftype == "LINE":
        ...
        fwrite(SVG_LINE.format(e.point[0], -e.point[1], e.point[0], -e.point[1], col, th, layername=layername))
    else:
...
if __name__ == "__main__":
    ...
    import re
    parser = OptionParser()
    parser.add_option("-d", "--dxf",        dest="dxfs",       metavar="FILE", action="append",   help="Input dxf file")
    parser.add_option("-s", "--svg",        dest="svg",        metavar="FILE",                    help="Output dxf file")
    parser.add_option("-v", "--version",    action="store_true")
    parser.description = "Convert (multiple) dxf to svg for use by laserplacer"
    ...
    if len(args) >= 1 and re.search("(?i)\.dxf$", args[0]) and not options.dxfs:
        options.dxfs = [ args.pop(0) ]
    ...
    if not options.dxfs:
        ...
        exit(1)
    if not options.svg: 
        options.svg = re.sub("(?i)\.dxf$", "", options.dxfs[0])+".svg"
    
    ds = [ dxfgrabber.readfile(dxf)  for dxf in options.dxfs ]
    ...
    minX = min(d.header['$EXTMIN'][0]  for d in ds)
    minY = max(d.header['$EXTMIN'][1]  for d in ds)
    maxY = max(d.header['$EXTMAX'][1]  for d in ds)
    tx = (0.0 if abs(minX) < 10000 else -minX)
    ty = (0.0 if abs(maxY) < 10000 else -minY - (maxY - minY))
    if tx != 0 or ty != 0:
        print("Moving origin by (%f %f)" % (tx, ty))

    fout = open(options.svg, "w")
    preamble(ds[0], fout.write, tx, ty)
    for d in ds:
